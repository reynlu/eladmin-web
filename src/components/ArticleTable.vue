  ...
  <div>
    ...
    >
      ...
      <el-table-column label="最近编辑时间" width="140" align="left">
        <template slot-scope="scope">{{ parseTime(scope.row.editTime) }}</template>
      </el-table-column>
      <el-table-column prop="user.nickName" label="作者" width="120" align="left" />
      <el-table-column prop="category.categoryName" label="所属分类" width="120" align="left" />
      <el-table-column v-if="showEdit || showDelete" label="操作" align="left">
        <template slot-scope="scope">
          <el-button v-if="showEdit" size="mini" @click="handleEdit(scope.$index, scope.row)">编辑</el-button>
          <el-button
            v-if="showRestore"
            size="mini"
...
export default {
  ...
  methods: {
    ...
    itemClick(row) {
      this.$router.push({ path: '/article/view', query: { aid: row.id }})
    },
    deleteMany() {
    ...
    loadBlogs(page, count) {
      ...
      _this.params.count = count
      articleApi.getArticle(_this.params).then(resp => {
        _this.loading = false
        _this.articles = resp.content
